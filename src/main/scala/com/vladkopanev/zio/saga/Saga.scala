...
final class Saga[-R, +E, +A] private (
  ...
  /**
    ...
    * */
  def flatten[R1 <: R, E1 >: E, B](
      implicit ev: A <:< Saga[R1, E1, B]): Saga[R1, E1, B] =
    self.flatMap(r => ev(r))
  ...
  /**
    ...
    * Both compensating actions would be executed in case of failure.
    * */
  def zipWithPar[R1 <: R, E1 >: E, B, C](that: Saga[R1, E1, B])(
      f: (A, B) => C): Saga[R1, E1, C] = {
  ...
  def zipWithParAll[R1 <: R, E1 >: E, B, C](
      ...
      g: (Compensator[R1, E1], Compensator[R1, E1]) => Compensator[R1, E1])
    : Saga[R1, E1, C] = {
    def coordinate[A1, B1, C1](f: (A1, B1) => C1)(
      ...
      fasterSaga.foldM(
        {
          cause =>
            def extractCompensatorFrom(
                c: Cause[(E1, Compensator[R1, E1])]): Compensator[R1, E1] =
              c.failures.headOption
                .map[Compensator[R1, E1]](_._2)
                .getOrElse(ZIO.dieMessage("Compensator was lost"))
            ...
            slowerSaga.await.flatMap(
              _.foldM(
                { loserCause =>
                  ...
                  ZIO.halt((cause && loserCause).map {
                    case (e, _) => (e, g(compB, compA))
                  })
                }, {
                  case (_, compB) =>
                    ZIO.halt(cause.map {
                      case (e, compA) => (e, g(compB, compA))
                    })
                }
              )
            )
  ...
  def transact: ZIO[R, E, A] =
    request
      .tapError({ case (e, compA) => compA.mapError(_ => (e, IO.unit)) })
      .bimap(_._1, _._1)
}
...
object Saga {
  ...
  /**
    ...
    * */
  def compensate[R, E, A](request: ZIO[R, E, A],
                          compensator: Compensator[R, E]): Saga[R, E, A] =
  ...
  /**
    ...
    * */
  def compensate[R, E, A](
      action: ZIO[R, E, A],
      compensation: Either[E, A] => Compensator[R, E]): Saga[R, E, A] =
    new Saga(
      action.bimap(e => (e, compensation(Left(e))),
                   a => (a, compensation(Right(a)))))
  ...
  /**
    ...
    * */
  def compensateIfFail[R, E, A](
      action: ZIO[R, E, A],
      compensation: E => Compensator[R, E]): Saga[R, E, A] =
    compensate(
      action,
      (result: Either[E, A]) => result.fold(compensation, _ => ZIO.unit))
  ...
  /**
    ...
    * */
  def compensateIfSuccess[R, E, A](
      action: ZIO[R, E, A],
      compensation: A => Compensator[R, E]): Saga[R, E, A] =
    compensate(
      action,
      (result: Either[E, A]) => result.fold(_ => ZIO.unit, compensation))
  ...
  def collectAllPar[R, E, A](
      sagas: Iterable[Saga[R, E, A]]): Saga[R, E, List[A]] =
  ...
  /**
    ...
    *
    */
  def foreachPar[R, E, A, B](as: Iterable[A])(
      fn: A => Saga[R, E, B]): Saga[R, E, List[B]] =
    as.foldRight[Saga[R, E, List[B]]](Saga.noCompensate(IO.succeed(Nil))) {
      (a, io) =>
  ...
  /**
    ...
    * */
  def noCompensate[R, E, A](request: ZIO[R, E, A]): Saga[R, E, A] =
    compensate(request, ZIO.unit)
  ...
  /**
    ...
    * */
  def succeed[R, A](value: A): Saga[R, Nothing, A] =
    noCompensate(ZIO.succeed(value))

  // $COVERAGE-OFF$
  implicit def IOtoCompensable[E, A](io: IO[E, A]): Compensable[Any, E, A] =
    new Compensable(io)

  implicit def ZIOtoCompensable[R, E, A](
      zio: ZIO[R, E, A]): Compensable[R, E, A] = new Compensable(zio)

  implicit def UIOtoCompensable[A](uio: UIO[A]): Compensable[Any, Nothing, A] =
    new Compensable(uio)

  implicit def RIOtoCompensable[R, A](
      rio: RIO[R, A]): Compensable[R, Throwable, A] = new Compensable(rio)

  implicit def TaskToCompensable[A](
      task: Task[A]): Compensable[Any, Throwable, A] = new Compensable(task)
  // $COVERAGE-ON$
  ...
  class Compensable[-R, +E, +A](val request: ZIO[R, E, A]) extends AnyVal {
    ...
    def compensate[R1 <: R, E1 >: E](
        compensation: Either[E1, A] => Compensator[R1, E1]): Saga[R1, E1, A] =
      Saga.compensate(request, compensation)

    def compensateIfFail[R1 <: R, E1 >: E](
        compensation: E1 => Compensator[R1, E1]): Saga[R1, E1, A] =
      Saga.compensateIfFail(request, compensation)

    def compensateIfSuccess[R1 <: R, E1 >: E](
        compensation: A => Compensator[R1, E1]): Saga[R1, E1, A] =
